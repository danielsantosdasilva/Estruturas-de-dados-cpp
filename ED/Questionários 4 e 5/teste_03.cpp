#include <iostream>
#include <vector>
#include <algorithm> // Incluindo o cabeçalho necessário para random_shuffle
    ...
    static long merge(vector<int>& arr, vector<int>& temp, int left, int mid, int right) {
        ...
        while (i <= mid && j <= right) {
            if (arr[i] <= arr[j]) {
                ...
                temp[k++] = arr[j++];
                inversions += mid - i + 1; // Conta as inversões
            }
        }
...
public:
    static void main() {
        ...
        // Criando uma lista de 1000 números inteiros distintos
        for (int i = 0; i < 1000; i++) {
            arr.push_back(i);
        }

        // Embaralhando a lista aleatoriamente usando random_shuffle
